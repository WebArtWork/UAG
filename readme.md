...
The node binary is **`uagd`**, used by full nodes and validators.
...
All commands use the `uagd` binary and the `muag` fee token.

1. **Store a contract** (any account can upload):

   ```bash
   uagd tx wasm store path/to/contract.wasm \
     --from <key-name> \
     --chain-id uag-test-1 \
...
2. **Instantiate** the uploaded code (replace `<code-id>` from the store result):
   ...
   ```bash
   uagd tx wasm instantiate <code-id> '{"count":0}' \
     --label "demo-counter" \
     --admin <admin-address> \
...
3. **Execute** a contract message:
   ...
   ```bash
   uagd tx wasm execute <contract-address> '{"increment":{}}' \
     --from <key-name> --chain-id uag-test-1 \
...
4. **Query** contract state:
   ...
   ```bash
   uagd query wasm contract-state smart <contract-address> '{"get_count":{}}'
   ```
